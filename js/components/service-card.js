class ServiceCard extends HTMLElement {
    connectedCallback() {
        ...
        let imgHtml = '';
        if (src) {
            imgHtml = `<img src="${src}" alt="${title}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.parentElement.style.background='#1a1a1a';">`;
        }

        this.innerHTML = `
      <div class="bg-[#111] border border-white/10 flex flex-col h-[381px] group m-4">
          <div class="h-[220px] w-full relative overflow-hidden bg-[#1a1a1a]">${imgHtml}</div>
